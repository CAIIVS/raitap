...
End-to-end flow of a `raitap` invocation, from CLI entry to report
generation. Useful when extending the CLI, the deps bootstrap, or the
pipeline; not necessary for users.
