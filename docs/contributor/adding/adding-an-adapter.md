...
- **`self._lazy_import()`.** Inherited from `AdapterMixin`. Imports `import_name` (or `f"{import_name}.{submodule}"` if you pass `submodule=`) at call time, keeping `import raitap` cheap and letting users install RAITAP without every wrapped library. Raises a clear install-hint `ImportError` if the library is missing.
...
- **`use: null` variants.** E.g. `reporting=disabled` carries `use: null` + `multirun_report: false`. The decorators always target a concrete class.
