...
### 4. Backend `task_kind` (auto-detection or explicit opt-in)

Backends have their own page: see [Adding a backend](adding-a-backend.md).

`src/raitap/models/backend.py:106` houses `_is_torchvision_detection_model`, a duck-typed check that runs at `TorchBackend.__init__` time. Either extend this helper for your model class, or pass the explicit `task_kind=` kwarg on `TorchBackend`:
