...
def test_csv_reads_text_column(tmp_path: Path) -> None:
    ...
    assert TextCsvInputParser(source=str(p), text_column="text").parse(
        source=str(p), sample_ids=None
    ) == ["hello", "world"]
...
def test_jsonl_reads_text_field(tmp_path: Path) -> None:
    ...
    assert TextJsonlInputParser(source=str(p)).parse(source=str(p), sample_ids=None) == ["a", "b"]


def test_dir_reads_txt_files_sorted(tmp_path: Path) -> None:
    (tmp_path / "1.txt").write_text("one")
    (tmp_path / "2.txt").write_text("two")
    assert TextDirInputParser(source=str(tmp_path)).parse(
        source=str(tmp_path), sample_ids=None
    ) == ["one", "two"]
