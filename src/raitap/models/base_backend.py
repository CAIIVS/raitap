...
class ModelBackend(ABC):
    ...
    # Runtime device for backends that place tensors (``TorchBackend`` sets this
    # in ``__init__``). ``None`` for device-less backends (ONNX, tree). Declared
    # here so callers read ``backend.device`` directly instead of via getattr.
